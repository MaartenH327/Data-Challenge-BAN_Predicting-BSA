...
*Change target to credit requirement of interest + make predictions
...
gen correct_prediction = (target == binary_predict)
*gen correct_prediction = (passed42 == binary_predict)

*Calculate accuracy and other scores

egen total_correct = total(correct_prediction)
gen total_incorrect = _N - total_correct

*generate TruePositives etc
gen TP = (target == 1 & binary_predict == 1)
gen FP = (target == 0 & binary_predict == 1)
gen TN = (target == 0 & binary_predict == 0)
gen FN = (target == 1 & binary_predict == 0)
